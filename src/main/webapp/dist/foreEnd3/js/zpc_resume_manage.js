            ...
            if(data.list.length!=0){
                ...
                for(var i=0;i<data.list.length;i++){
                    ...
                    if(data.list[i].sendState==1){           //意向沟通
                        ...
                        var sj=bb-aa;
                        str+='<td class="all_no">'+sj+'</td>'           //页面负责运算计算年龄
                        str+='<td class="all_no">'+data.list[i].resumes.resumePosition+'</td>' //页面负责运算计算年龄
...
Qb.prototype.seekCont=function (parent){
    ...
    _public_ssk.resumeName=parent1.find('input').eq(0).val()                   //姓名

    _public_ssk.resumeWorkspace=parent2.find('input').eq(0).val();               //所在地
    _public_ssk.ageMin=parent2.find('input').eq(1).val()                      //年龄   小
    _public_ssk.ageMax=parent2.find('input').eq(2).val()                   //年龄   大
