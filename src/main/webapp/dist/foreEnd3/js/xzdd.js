...
function didian(obj1,obj2,cgzx){
    $(obj1).on('click',function (){
        ...
        $("#zp_gsxq_bd_tck .zp_ul a").each(function (index,e){      //载入成功
            $(e).on('click',function (event){
                event.stopPropagation();                             //取消冒泡
                var str=''
                str+='<ul>'
                str+='<h4 class="text-center">'+$(this).html()+'</h4>';
                for(var i=0;i<map[index].length;i++){
                    str+='<li><a href="#0" class="cd-popup-trigger" >'+map[index][i]+'</a></li>'
                }
                str+='</ul>'
                ...
                $('.yp_cs ul a').each(function (i,e2){              //省市点击事件
                    $(e2).on('click',function () {
                        $('#div6').html($(this).html());
                        $('.yp_cs').css({"display":"none"}).html('');
                    })

                });

                $('#zp_gsxq_bd_tck').unbind().on('click',function (){                 //点旁边关掉
                    $('.yp_cs').css({"display":"none"}).html('');
                })
        ...
        $('#zp_gsxq_bd_tck .zp_button1').unbind().on('click',function () {           //确定保存事件
            if($('#div6').html()!=''&&$('#div6').html()!=null){
